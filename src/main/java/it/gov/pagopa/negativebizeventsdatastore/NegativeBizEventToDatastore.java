...
public class NegativeBizEventToDatastore {
    ...
    public void processBizEvent (
        ...
        String message = String.format("NegativeBizEventToDatastore function called at %s with events list size %s and properties size %s", LocalDateTime.now(), negativeBizEvtMsg.size(), properties.length);
        logger.info(message);
        ...
        try {
            if (negativeBizEvtMsg.size() == properties.length) {
                ...
                throw new AppException("Error during processing - "
                                               ...
                                               "[bizEvtMsg.size=" + negativeBizEvtMsg.size() + "; properties.length=" + properties.length + "]");
            }
        } catch (Exception e) {
